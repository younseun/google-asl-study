...
def dnn_model(features, mode, params):
    ...
    h2 = tf.layers.dense(h1, 5, activation=tf.nn.relu)
...
def rnn_lstm_model(features, mode, params):
    CELL_SIZE = N_INPUTS // 3  # size of the internal state in each of the cells
    ...
    # 3. pass rnn output through a dense layer
    h1 = tf.layers.dense(state[1], N_INPUTS //2, activation=tf.nn.relu)
    predictions = tf.layers.dense(h1, 1, activation=None)
    return predictions
